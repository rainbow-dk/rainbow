...
public class ValidCodeUtils {
	 ...
	 */
	public int COLOR_FC_BG = 200;
	/***
	 * 验证码背景颜色COLOR_FC_BG 应当小于COLOR_BC_BG
	 */
	public int COLOR_BC_BG = 250;

	/**********************************************************************
	 * 验证码背景干扰线颜色COLOR_FC_LINE 应当小于COLOR_BC_LINE
	 */
	public int COLOR_FC_LINE = 160;
	/***
	 * 验证码背景干扰线颜色COLOR_FC_LINE 应当小于COLOR_BC_LINE
	 */
	public int COLOR_BC_LINE = 200;

	/***************************************************************************
	 * 验证码颜色COLOR_FC_CODE 应当小于COLOR_BC_CODE
	 */
	public int COLOR_FC_CODE = 20;
	/***
	 * 验证码颜色COLOR_FC_CODE 应当小于COLOR_BC_CODE
	 */
	public int COLOR_BC_CODE = 170;
	...
	public static Boolean Valid(HttpServletRequest request, String inputCode)
	{
		if(null == request.getSession().getAttribute(sessionCode()))
		{
			return false;
		}
		return Valid(request.getSession().getAttribute(sessionCode()).toString(), inputCode);
	}
	
	public static Boolean Valid(String constCode, String inputCode)
	{
		if(null == constCode || !constCode.equalsIgnoreCase(inputCode.trim())){
			return false;
		}
		return true;
	}
	...
	public String getImage(HttpServletRequest request,
		...
		for (int i = 0; i < 5; i++) {
			...
			img.drawString(rand, 18 * i + 25, 30);
		}
		request.getSession().setAttribute(sessionCode(), codeValue);
		// 图象生效
		img.dispose();
		// 输出图象到页面
	...
	public Font getRandomFont() {
		...
		int fontSize = (int) Math.round(Math.random() * 5 + 26);
